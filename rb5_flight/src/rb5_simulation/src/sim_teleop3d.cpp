...
public:
    TeleopDrone()
    {
        ...
        cmd_vel_pub_ = nh.advertise<geometry_msgs::TwistStamped>("mavros/local_position/velocity_local", 10);
...
private:
    ...
    bool setOffboardMode() {
        ...
        while (ros::ok()) {
            ...
            // If already in OFFBOARD mode and armed, we can proceed
    ...
    void startControlLoop() {        
        ...
        while (ros::ok())   
        {
            ...
            switch (ch)
            {
                case 'w': cmd_vel.twist.linear.x = 0.5; break;   // Move forward
                case 's': cmd_vel.twist.linear.x = -0.5; break;  // Move backward
                case 'a': cmd_vel.twist.linear.y = 0.5; break;   // Move left
                case 'd': cmd_vel.twist.linear.y = -0.5; break;  // Move right
                case 'r': cmd_vel.twist.angular.z = 0.5; break;   // Rotate clockwise
                case 'f': cmd_vel.twist.angular.z = -0.5; break;  // Rotate counter-clockwise
                case 'u': cmd_vel.twist.linear.z = 0.5; break;   // Move up
                case 'j': cmd_vel.twist.linear.z = -0.5; break;  // Move down
...
{
    ros::init(argc, argv, "sim_teleop_node");
    TeleopDrone teleop;
    ros::spin(); 
    return 0;
}
